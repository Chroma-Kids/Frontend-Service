...
const TableRowStudent = (props) => {
  ...
  return (
    (!!student &&
      <tr>
          <td>{}</td>
          <td><Link to={ROUTES.AUTHENTICATED.STUDENT(studentKey)}>{student.name} {student.surname}</Link></td>
          ...
          <td><button className="btn btn-danger btn-sm" onClick={deleteStudentFromClassroom.bind(this, classroomId, studentKey)}>Remove</button></td>
      </tr>)
  )
}
